...
class RetinoblastoGemmaV6:
    ...
    def initialize_modules_async(self):
        ...
        def init_thread():
            try:
                if hasattr(self, 'modern_ui'):
                    self.modern_ui.update_status_badge("🔄 LOADING MODULES", "warning")

                # Après Gemma chargé avec succès:
                if hasattr(self, 'modern_ui'):
                    self.modern_ui.update_status_badge("✅ SYSTEM READY", "success")

                # En cas d'erreur:
                if hasattr(self, 'modern_ui'):
                    self.modern_ui.update_status_badge("❌ ERROR", "error")
    ...
    def update_module_status(self, module_key: str, text: str, color: str):
        ...
        # Interface moderne
        if hasattr(self, 'modern_ui'):
            self.modern_ui.update_module_status_modern(module_key, text, color)
        # Interface classique
        elif hasattr(self, 'module_status') and module_key in self.module_status:
    ...
    def analyze_image(self):
        ...
        # Confirmation pour analyse (version moderne)
        if hasattr(self, 'modern_ui'):
            self.modern_ui.show_analysis_confirmation(self._start_analysis_process)
            return
        else:
            # Version classique
    ...
    def _start_analysis_process(self):
        """Lance effectivement le processus d'analyse"""
        def analysis_thread():
